...
Next session starts here: M3 is DONE except the rough demo recording, which you make (script under 'Demo recording' below; record the file's location there). Conflicts are live: rule-based claims, deterministic predicate and selection, `/conflicts`, the inspector on `https://main.d1jy52bqj8dt1h.amplifyapp.com/app/?ws=ws_KFdHFNj0IPUoOs4pQDcMUQ` (ADR-020; precision and recall 1.0, BENCHMARKS). Next: M4 (timeline, polish, reliability), `prompts/04-M4-TIMELINE-POLISH-RELIABILITY.md`. Open: B10 (gitleaks summaries, signed in); ADR-019 gates to confirm. Deploys pass every parameter in HANDOFF, including AllowedOrigins.
...
## Demo recording (M3 done-means; you record it)
