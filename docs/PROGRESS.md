...
Next session starts here: Bedrock is skipped (ADR-017): production answers with Groq `openai/gpt-oss-120b` (fallback `gpt-oss-20b`) and embeds with local ONNX bge-small; OpenSearch stays. M2's Offline Gate is GREEN again with the Groq, ONNX, workflow and observability additions (`docs/CHECKLIST.md`). Next, the M2 Live Gate: (1) you create the SSM SecureString `/crownx/groq-api-key`; (2) `python scripts/fetch_models.py --publish <bucket>`, then `sam build` and `sam deploy` with the printed sha256 values (ask first); (3) seed with `demo/seed.py`, run `evals/run.py --api <ApiUrl> --pace 2.5`, record the live row and gates; then S6 (Amplify). Also open: B10. Handoff notes: `docs/HANDOFF.md`.
...
| B11 | The Groq API key must exist as an SSM SecureString before the Live Gate deploy. Claude never handles the key. Command (run it yourself, in your own terminal): `aws ssm put-parameter --region ap-south-1 --name /crownx/groq-api-key --type SecureString --value <your key>` | you | the parameter exists (`aws ssm describe-parameters --parameter-filters Key=Name,Values=/crownx/groq-api-key`) |
| B10 | CI's gitleaks job flagged commit `6ac4715` (slice D). The job summary needs a GitHub sign-in, so the rule and line aren't known here; later pushes pass because gitleaks scans only each push's commits. Likely a fake test key (`gsk_secret`) or a config line. History can't be rewritten (rule 7) | you, then Claude | you read the job summary of run 35337564630 signed in; Claude adds the fingerprint to `.gitleaksignore` if it's a false positive, or rotates and removes the value if it isn't |
